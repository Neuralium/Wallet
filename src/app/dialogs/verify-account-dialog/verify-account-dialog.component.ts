...
export class VerifyAccountDialogComponent implements OnInit, OnDestroy {
  ...
  form1: FormGroup;
  hideBypassVerification:boolean = false;
  hideAppointments:boolean = false;
  verificationSelection = 1; // 0;
  
  hideTHS:boolean = true;
  ...
  ngOnInit() {
    ...
    this.hideBypassVerification = AppConfig.production && !AppConfig.productiontest;
    
    this.walletService.getCurrentAccount().pipe(takeUntil(this.unsubscribe$)).subscribe(account => {
      this.account = account;

      this.hideTHS = account.accountType !== WalletAccountType.Server;
      this.hideAppointments = account.accountType !== WalletAccountType.User
    });
  ...
  displayHelpText(){
    ...
    if(this.verificationSelection === 1){
      ...
      if(this.thsHelpText === ''){
        this.translateService.get('verifyAccount.THSHelpText').subscribe(text => {
          this.thsHelpText = text;
          this.helpText = text;
        });
      }
      else{
        this.helpText = this.thsHelpText;
      }
    }
  }
