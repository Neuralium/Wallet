...
getMessages(): Array<ServerMessage> {
  ...
  startListeningPOWEvents() {
    this.listenToPOWBegin();
    this.listenToPOWIteration();
    this.listenToPOWSolution();
  }
  ...
  callQueryNeuraliumTimelineSection(accountCode: string, firstday:  DateTime, skip: number, take: number) {
    const service = NeuraliumCall.create(this, this.logService);
    return service.callQueryNeuraliumTimelineSection(accountCode, firstday, skip, take);
  }
  ...
  listenToMessage() {
    ...
    this.registerConnectionEvent(action, (message: string, timestamp:  DateTime, level:string, properties:Array<Object>) => {
      ...
      this.propagateEvent(1, EventTypes.Message, ResponseResult.Success, NullBlockchainType, message);
    });
  }
  ...
  listenToPOWBegin(){
    const cnx = this.connection;
    const action = 'POWBegin';

    this.registerConnectionEvent(action, (chainType: number, difficulty:number) => {
      this.logEvent(action + ' - event', { 'chainType': chainType, 'difficulty' : difficulty });
      this.propagateEvent(chainType, EventTypes.POWBegin, ResponseResult.Success, chainType,{'chainType': chainType, 'difficulty' : difficulty});
    });
  }

  listenToPOWIteration(){
    const cnx = this.connection;
    const action = 'POWIteration';

    this.registerConnectionEvent(action, (chainType: number, nonce:number, difficulty:number) => {
      this.logEvent(action + ' - event', { 'chainType': chainType, 'nonce' : nonce, 'difficulty' : difficulty });
      this.propagateEvent(chainType, EventTypes.POWIteration, ResponseResult.Success, chainType,{'chainType': chainType, 'nonce' : nonce, 'difficulty' : difficulty});
    });
  }

  listenToPOWSolution(){
    const cnx = this.connection;
    const action = 'POWSolution';

    this.registerConnectionEvent(action, (chainType: number, nonce:number, solution:number, difficulty:number) => {
      this.logEvent(action + ' - event', { 'chainType': chainType, 'nonce' : nonce, 'solution' : solution, 'difficulty': difficulty });
      this.propagateEvent(chainType, EventTypes.POWSolution, ResponseResult.Success, chainType,{'chainType': chainType, 'nonce' : nonce, 'solution' : solution, 'difficulty' : difficulty});
    });
  }
