...
export class NeuraliumCall extends CommonCall {
            ...
            this.serviceConnectionService.invoke<Array<object>>('QueryNeuraliumTimelineHeader', accountCode)
                .then(
                    response => {
                        ...
                        const firstDay = DateTime.fromISO(response['firstDay']).toUTC();
    ...
    callQueryNeuraliumTimelineSection(accountCode: string, firstday: DateTime, skip: number, take: number): Promise<Array<TimelineDay>> {
        ...
        return new Promise<Array<TimelineDay>>((resolve, reject) => {
            ...
            if (!firstday.isValid) {
                resolve(new Array<TimelineDay>());
                return;
            }

            this.logEvent('QueryNeuraliumTimelineSection - call', { accountCode, firstday, skip, take });
            this.serviceConnectionService.invoke<Array<object>>('QueryNeuraliumTimelineSection', accountCode, firstday, skip, take)
                .then(
                    response => {
                        ...
                        response.forEach(element => {
                            ...
                            const utcDay = DateTime.fromISO(element['day']).toUTC();
                            const Day: DateTime = DateTime.fromObject({ day: utcDay.day, month: utcDay.month, year: utcDay.year });
                            ...
                            Entries.forEach(item => {
                                ...
                                const Timestamp = DateTime.fromISO(item['timestamp']).toUTC();
