...
import { WalletAccount, WalletAccountStatus, WalletAccountType, AccountCanPublish, AccountAppointmentConfirmationResult, AccountPublicationModes } from '../..//model/walletAccount';
        ...
        .then(
          response => {
            ...
            response.forEach(account => {
              ...
              const correlated: number = <number>account.correlated;
              ...
              const walletAccount = WalletAccount.createNew(accountCode, accountId, status, 0, correlated, accountType, friendlyName, false, isActive,inAppointment);
      ...
      this.serviceConnectionService.invoke<Array<WalletAccount>>('QueryWalletAccountDetails', chainType, accountCode)
        .then(
          account => {
            ...
            const correlated = <number>account['correlated'];
            ...
            const walletAccount = WalletAccount.createNew(accountCodeReceived, accountId, status, declarationBlockId, correlated, accountType, friendlyName, isEncrypted, isActive, inAppointment, accountHash, trustLevel);
